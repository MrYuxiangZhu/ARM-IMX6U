...
void clk_init(void)
{
	uint32_t reg = 0;
	/* 设置ARM内核时钟为528MHz */
	/* 判断当前ARM内核是使用的那个时钟源启动的，正常情况下ARM内核是由pll1_sw_clk驱动的，而
	...
	if (0 == GPIO_PIN_READ(CCM_CCSR, 2))	/* 当前pll1_sw_clk使用的pll1_main_clk*/
	{
		CCM_CCSR &= ~(1 << 8);				/* 配置step_clk时钟源为24MH OSC */	
	 ...
	 * bit13: 1 使能时钟输出
	 * bit[6:0]: 88, 由公式：Fout = Fin * div_select / 2.0，1056=24*div_select/2.0, 得出：div_select=88  
	 */
	CCM_ANALOG_PLL_ARM = (1 << 13) | ((88 << 0) & 0X7F); 	/* 配置pll1_main_clk=1056MHz */
	CCM_CCSR &= ~(1 << 2);									/* 将pll_sw_clk时钟重新切换回pll1_main_clk */
	CCM_CACRR = 0X1;										/* ARM内核时钟为pll1_sw_clk/2=1056/2=528Mhz */

	/* 设置PLL2(SYS PLL)各个PFD */
	reg = CCM_ANALOG_PFD_528;
	reg &= ~(0X3F3F3F3F);		/* 清除原来的设置 				 */
	reg |= 32 << 24;			/* PLL2_PFD3=528*18/32=297Mhz 	*/
	reg |= 24 << 16;			/* PLL2_PFD2=528*18/24=396Mhz(DDR使用的时钟，最大400Mhz) */
	reg |= 16 << 8;				/* PLL2_PFD1=528*18/16=594Mhz 	*/
	reg |= 27 << 0;				/* PLL2_PFD0=528*18/27=352Mhz  	*/
	CCM_ANALOG_PFD_528 = reg;	/* 设置PLL2_PFD0~3 		 		*/

	/* 设置PLL3(USB1)各个PFD */
	reg = 0;					/* 清零   */
	reg = CCM_ANALOG_PFD_480;
	reg &= ~(0X3F3F3F3F);		/* 清除原来的设置 					 */
	...
	CCM_CBCDR &= ~(7 << 10);	/* CBCDR的AHB_PODF清零 */
	CCM_CBCDR |= 2 << 10;		/* AHB_PODF 3分频，AHB_CLK_ROOT=132MHz */
	while(CCM_CDHIPR & (1 << 1));/* 等待握手完成 */
#endif
